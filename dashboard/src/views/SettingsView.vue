<template>
  <div class="space-y-8">
    ...
    <div class="bg-white shadow-sm rounded-xl border border-gray-200 overflow-hidden">
      <div class="px-6 py-5 bg-gray-50 border-b border-gray-200">
        <h3 class="text-lg leading-6 font-bold text-gray-900">إعدادات الرواتب والحضور والإجازات</h3>
        <p class="text-xs text-gray-500 mt-1">أي تغيير هنا يؤثر على إصدار الرواتب القادم فقط.</p>
      </div>
      <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
        <div v-for="item in [
          ['overtime_bonus_per_minute','معامل الإضافي'],
          ['late_deduction_per_minute','معامل خصم التأخير'],
          ['late_grace_minutes','دقائق السماح للتأخير'],
          ['earliest_checkin_minutes','السماح بالحضور المبكر (دقائق)'],
          ['late_absence_threshold_minutes','حد تحويل التأخير إلى غياب (دقائق)'],
          ['monthly_paid_leave_days','الإجازة المدفوعة الشهرية (أيام)'],
          ['annual_leave_quota','الرصيد السنوي الأساسي (أيام)'],
          ['absence_deduction_per_day','خصم يوم الغياب'],
          ['attendance_bonus_threshold_days','حد الحضور لمكافأة الإجازة'],
          ['attendance_bonus_leave_days','أيام الإجازة الإضافية']
        ]" :key="item[0]">
          <label class="block text-sm font-semibold text-gray-700 mb-1">{{ item[1] }}</label>
          <div class="flex gap-3">
            <input type="number" min="0" step="0.01" v-model="settingsData[item[0]]" class="block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 sm:text-sm">
            <button @click="saveSetting(item[0], settingsData[item[0]])" class="px-4 py-2 rounded-md text-sm font-medium text-white bg-primary-600 hover:bg-primary-700">حفظ</button>
          </div>
        </div>
        <div>
          <label class="block text-sm font-semibold text-gray-700 mb-1">تفعيل خصم الغياب</label>
          <div class="flex gap-3">
            <select v-model="settingsData.absence_deduction_enabled" class="block w-full border border-gray-300 rounded-md py-2 px-3 sm:text-sm">
              <option value="0">مغلق</option>
              <option value="1">مفعل</option>
            </select>
            <button @click="saveSetting('absence_deduction_enabled', settingsData.absence_deduction_enabled)" class="px-4 py-2 rounded-md text-sm font-medium text-white bg-primary-600 hover:bg-primary-700">حفظ</button>
          </div>
        </div>
      </div>
    </div>
